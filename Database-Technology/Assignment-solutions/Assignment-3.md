...
1. Write a query to display the first_name, last_name using alias name “First Name", "Last Name".
        ...
        Ans:    d3_56456> SELECT FIRST_NAME AS 'First Name', LAST_NAME AS 'Last Name'   FROM employees;
                           +-------------+-------------+
...
2. Write a query to get the names (first_name, last_name), salary, PF of all the employees (PF is calculated as 15% of salary).
        ...
        Ans:        SELECT CONCAT(FIRST_NAME," ",LAST_NAME) AS NAME, SALARY, SALARY*1.15 AS PF FROM employees;
                           +-------------------+----------+-----------+
                           | NAME              | SALARY   | PF        |
                           +-------------------+----------+-----------+
...
3. Write a query to select first 10 records from a employees table.
        ...
        Ans:        d3_56456> SELECT * FROM employees LIMIT 10;
                           +-------------+------------+-----------+----------+--------------+------------+------------+----------+----------------+------------+---------------+
...
4. Write a query to display job id and job title of first 5 jobs.
        ...
        Ans:        d3_56456> SELECT JOB_ID, JOB_TITLE FROM jobs LIMIT 5;
                           +------------+-------------------------------+
                           | JOB_ID     | JOB_TITLE                     |
                           +------------+-------------------------------+
                           | AD_PRES    | President                     |
...
5. Write a query to display location id, street address and postal code of 6 locations after first 3 locations.
        ...
        Ans:        d3_56456> SELECT LOCATION_ID, STREET_ADDRESS, POSTAL_CODE FROM locations LIMIT 3, 6;
                           +-------------+---------------------+-------------+
...
6. Write a query to display job title and difference between max and min salary for that job.
        ...
        Ans:        d3_56456> SELECT JOB_TITLE, MIN_SALARY, MAX_SALARY , (MAX_SALARY - MIN_SALARY) AS SALARY_DIFFERENCE    FROM jobs ;
                           +---------------------------------+------------+------------+-------------------+
                           | JOB_TITLE                       | MIN_SALARY | MAX_SALARY | SALARY_DIFFERENCE |
                           +---------------------------------+------------+------------+-------------------+
...
7. Write a query to get unique department ID from employee table.
        ...
        Ans:        d3_56456> SELECT DISTINCT DEPARTMENT_ID FROM employees;
                           +---------------+
                           | DEPARTMENT_ID |
                           +---------------+
                           |             0 |
...
8. Write a query to get all employee details from the employee table order by first name, descending.
        ...
        Ans:   d3_56456> SELECT * FROM employees ORDER BY FIRST_NAME DESC;
                  +-------------+-------------+-------------+----------+--------------------+------------+------------+----------+----------------+------------+---------------+
                  | EMPLOYEE_ID | FIRST_NAME  | LAST_NAME   | EMAIL    | PHONE_NUMBER       | HIRE_DATE  | JOB_ID     | SALARY   | COMMISSION_PCT | MANAGER_ID | DEPARTMENT_ID |
                  +-------------+-------------+-------------+----------+--------------------+------------+------------+----------+----------------+------------+---------------+
...
salary in ascending order of salary.
        ...
        Ans:  d3_56456> SELECT EMPLOYEE_ID, CONCAT(FIRST_NAME," ",LAST_NAME) AS NAMES, SALARY FROM employees ORDER BY SALARY ASC;
                  +-------------+-------------------+----------+
...
10. Display first name and join date of the employees who is either IT Programmer or Sales Man.
        ...
        Ans:    d3_56456> SELECT FIRST_NAME, HIRE_DATE , JOB_ID FROM employeeS WHERE JOB_ID IN('IT_PROG', 'SA_MAN');
                    +------------+------------+---------+
...
11. Display details of employee with ID 150 or 160.
        ...
        Ans:    d3_56456> SELECT * FROM employees WHERE EMPLOYEE_ID IN (150, 160);
                    +-------------+------------+-----------+---------+--------------------+------------+--------+----------+----------------+------------+---------------+
                    | EMPLOYEE_ID | FIRST_NAME | LAST_NAME | EMAIL   | PHONE_NUMBER       | HIRE_DATE  | JOB_ID | SALARY   | COMMISSION_PCT | MANAGER_ID | DEPARTMENT_ID |
                    +-------------+------------+-----------+---------+--------------------+------------+--------+----------+----------------+------------+---------------+
                    |         150 | Peter      | Tucker    | PTUCKER | 011.44.1344.129268 | 1987-08-06 | SA_REP | 10000.00 |           0.30 |        145 |            80 |
...
12. Display first name, salary, commission pct, and hire date for employees with salary less than 10000.
        ...
        Ans:    d3_56456> SELECT FIRST_NAME, SALARY, COMMISSION_PCT, HIRE_DATE FROM employees WHERE SALARY < 10000;
                    +-------------+---------+----------------+------------+
...
13. Display details of jobs in the descending order of the title.
        ...
        Ans:    d3_56456> SELECT * FROM jobs ORDER BY JOB_TITLE DESC;
                    +------------+---------------------------------+------------+------------+
...
14. Display details of the employees where commission percentage is null and salary in the range 5000 to 10000 and department is 30.
        ...
        Ans:    d3_56456> SELECT * FROM employees WHERE COMMISSION_PCT IS NULL AND (SALARY > 5000 AND SALARY < 10000) AND DEPARTMENT_ID = 30;
                          Empty set (0.00 sec)

                d3_56456> SELECT * FROM employees WHERE COMMISSION_PCT IS NULL AND  SALARY BETWEEN (5000 AND 10000) AND DEPARTMENT_ID = 30;
                          Empty set (0.00 sec)


15. Display employees first_name,email who are working in “Executive” department.

        Ans:    d3_56456>SELECT FIRST_NAME, EMAIL FROM employees WHERE DEPARTMENT_ID = (SELECT DEPARTMENT_ID FROM departments WHERE DEPARTMENT_NAME='Executive');
                        +------------+----------+
                        | FIRST_NAME | EMAIL    |
                        +------------+----------+
                        | Steven     | SKING    |
...
16. Display unique contry_id from locations table.
        ...
        Ans:    d3_56456>SELECT DISTINCT COUNTRY_ID FROM locations;
                    +------------+
                    | COUNTRY_ID |
                    +------------+
                    | "          |
...
17. Display all employees whose have job_id IT_PROG and FI_ACCOUNT.
        ...
        Ans:    d3_56456> SELECT * FROM employees WHERE JOB_ID = 'IT_PROG' OR JOB_ID ='FI_ACCOUNT';
                        +-------------+-------------+-----------+----------+--------------+------------+------------+---------+----------------+------------+---------------+
                        | EMPLOYEE_ID | FIRST_NAME  | LAST_NAME | EMAIL    | PHONE_NUMBER | HIRE_DATE  | JOB_ID     | SALARY  | COMMISSION_PCT | MANAGER_ID | DEPARTMENT_ID |
                        +-------------+-------------+-----------+----------+--------------+------------+------------+---------+----------------+------------+---------------+
                        |         109 | Daniel      | Faviet    | DFAVIET  | 515.124.4169 | 1987-06-26 | FI_ACCOUNT | 9000.00 |           0.00 |        108 |           100 |
...
18. Display all countries in ascending order.
        ...
        Ans:    d3_56456>SELECT * FROM countries ORDER BY COUNTRY_NAME ASC;
                        +------------+--------------------------+-----------+
...
19. Display employees where first name or last name start with S.
        ...
        Ans:    d3_56456>SELECT * FROM employees WHERE FIRST_NAME LIKE 'S%' OR LAST_NAME LIKE 'S%';
                        +-------------+------------+-----------+----------+--------------------+------------+------------+----------+----------------+------------+---------------+
                        | EMPLOYEE_ID | FIRST_NAME | LAST_NAME | EMAIL    | PHONE_NUMBER       | HIRE_DATE  | JOB_ID     | SALARY   | COMMISSION_PCT | MANAGER_ID | DEPARTMENT_ID |
                        +-------------+------------+-----------+----------+--------------------+------------+------------+----------+----------------+------------+---------------+
                        |         100 | Steven     | King      | SKING    | 515.123.4567       | 1987-06-17 | AD_PRES    | 24000.00 |           0.00 |          0 |            90 |
...
22. Display all the Part names with weight between 12 and 14 (inclusive of both).
        ...
        Ans:    d3_56456> SELECT * FROM P WHERE WEIGHT BETWEEN 12 AND 14;
                        +------+-------+-------+--------+--------+
...
24. Display all the suppliers with a status between 10 and 20.
        ...
        Ans:    d3_56456>SELECT * FROM S WHERE Status BETWEEN 10 AND 20;
                        +------+--------+--------+--------+
                        | S#   | Sname  | Status | City   |
                        +------+--------+--------+--------+
                        | S1   | Smith  |     20 | London |
...
26. Display all the suppliers from London.
        ...
        Ans:    d3_56456>SELECT Sname FROM S WHERE City = 'London';
                        +--------+
                        | Sname  |
                        +--------+
                        | Smith  |
...
27. Display all suppliers from Paris or London.
        ...
        Ans:    d3_56456> SELECT Sname FROM S WHERE City IN ('Paris', 'London');
                        +--------+
                        | Sname  |
                        +--------+
                        | Smith  |
...
28. Display all project in Athens.
        ...
        Ans:    d3_56456> SELECT Jname FROM J WHERE CITY = 'Athens';
                        +---------+
                        | Jname   |
                        +---------+
                        | Reader  |
